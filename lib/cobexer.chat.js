...
$.widget("cobexer.chat", {
	...
	_writeMessage: function(list, message) {
		...
		if (Array == message.constructor) {
			...
			for(var i = 0; i < message.length; i++) {
				var $msg = $('<div></div>');
				$msg.append($('<span></span>').text(this.options.comm.formatTimestamp(message[i].timestamp)));
			...
			var $msg = $('<div></div>');
			$msg.append($('<span></span>').text(this.options.comm.formatTimestamp(message.timestamp)));
	...
	_send: function(e) {
		...
		if (s) {
			this._trigger('send', { channel: this.options.channels[this.state.channel].name, message: s });
	...
	_toggleOnline: function() {
		if (this.state.online) {
			...
			this._trigger('connect', channels);
		}
	},
